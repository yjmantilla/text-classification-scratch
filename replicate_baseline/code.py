...
if __name__ == '__main__':
    ...
    data_path = 'Y:/code/text-classification-scratch/data'
    root_path=os.path.dirname(r'Y:\code\text-classification-scratch\replicate_baseline\code.py')
    ...
    #make dataframe
    if not os.path.exists(os.path.join(root_path,'binary_counts.csv')):
        ...
        df_binary.sort_values(by='count1',ascending=False).to_csv(os.path.join(root_path,'binary_counts.csv'),index=False)

    if not os.path.exists(os.path.join(root_path,'binary_counts_class.csv')):
        ...
        df_binary=pd.DataFrame(co)
        df_binary.sort_values(by=['count0@False'],ascending=[False]).to_csv(os.path.join(root_path,'binary_counts_class.csv'),index=False)

    df_binary=pd.read_csv(os.path.join(root_path,'binary_counts_class.csv'))

    df_binary.sort_values(by=['count0@False'],ascending=[True]).to_csv(os.path.join(root_path,'binary_counts_class_sorted.csv'),index=False)
    ...
    df_sel.sort_values(by=['ratio1TF'],ascending=[False]).to_csv(os.path.join(root_path,'binary_counts_class_sorted_sel.csv'),index=False)
    ...
    df_nsel.sort_values(by=['ratio10FT'],ascending=[False]).to_csv(os.path.join(root_path,'binary_counts_class_sorted_nsel.csv'),index=False)
    ...
    if not os.path.exists(os.path.join(root_path,'word_power.npy')):
        ...
        np.save(os.path.join(root_path,'word_power.npy'), word_power)
        np.save(os.path.join(root_path,'index_power.npy'), index_power)
    else:
        word_power=np.load(os.path.join(root_path,'word_power.npy'), allow_pickle=True).item()
        index_power=np.load(os.path.join(root_path,'index_power.npy'), allow_pickle=True)
    ...
    if not os.path.exists(os.path.join(root_path,'vocab_map.csv')):
        df = pd.DataFrame(vocab_map)
        df.to_csv(os.path.join(root_path,'vocab_map.csv'), index=False)
    ...
    df.to_csv(os.path.join(root_path,'submission_P.csv'), index=False)
    preds_test=(N_test<1).astype(int)
    df = pd.DataFrame({'ID': np.arange(len(preds_test)), 'label': preds_test})
    df.to_csv(os.path.join(root_path,'submission_N.csv'), index=False)
    ...
    for model_,mod in zip([LogisticRegression,LinearRegression,Perceptron,SVM],['LogR','LR','Per','SVM']):
        ...
        if not os.path.exists(os.path.join(root_path,f'best_args_{mod}.npy')):
            best_args, best_error,results,combs = opt(model_, regs, stepsizes, iters)
            reg=best_args[0]
            stepsize=best_args[1]
            iters=best_args[2]
            np.save(os.path.join(root_path,f'best_args_{mod}.npy'), best_args)
            np.save(os.path.join(root_path,f'best_error_{mod}.npy'), best_error)
            np.save(os.path.join(root_path,f'results_{mod}.npy'), [r for x,r in results])
            np.save(os.path.join(root_path,f'combs_{mod}.npy'), combs)

        best_error=1#np.load(f'best_error_{mod}.npy')
        #best_args=np.load(f'best_args_{mod}.npy')
        results=np.load(os.path.join(root_path,f'results_{mod}.npy'),allow_pickle=True)
        combs=np.load(os.path.join(root_path,f'combs_{mod}.npy'),allow_pickle=True)
        ...
        for i,r in enumerate(results):
            ...
            df.to_csv(os.path.join(root_path,f'submission_{mod}_{i}.csv'), index=False)
            ...
            df = pd.DataFrame({'ID': np.arange(len(preds)), 'label': preds.astype(int)})
            df.to_csv(os.path.join(root_path,f'known_{mod}_{i}_{error_rate}.csv'), index=False)
            df = pd.DataFrame({'ID': testset+valset, 'label': (labels_train_sign[testset+valset]>0).astype(int)})
            df.to_csv(os.path.join(root_path,f'known_{mod}_{i}_true.csv'), index=False)

    # load predictions from the models
    import numpy as np
    import pandas as pd
    import glob
    results=glob.glob(os.path.join(root_path,'submission_*_*.csv'))
    answers=glob.glob(os.path.join(root_path,'known*true.csv'))
    checks=glob.glob(os.path.join(root_path,'known*'))
    ...
    df.to_csv(os.path.join(root_path,f'submission_ensemble.csv'), index=False)
